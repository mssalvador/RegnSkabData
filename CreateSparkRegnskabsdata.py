...
removeBlanksDf = (df
                  .filter(df["id"] != np.NaN)
                  .select(df["name"],df["id"],F.regexp_replace(df["value"], r'(\s+) ',"").alias("value"),df["startDate"],df["endDate"])) # removes empty lines such as titles
...
specialPivotDf = (removeBlanksDf
                  .dropDuplicates()
                  .groupby("id","name","value","startDate","endDate")
                  .count()
                  .groupby("id")
                  .pivot("name")
...
transformedDf.write.csv(cleanedCsvPath+"/pivotRowDataValues.csv", sep=";", header=True)
pivotCountDf.write.csv(cleanedCsvPath+"/pivotRowDataCounts.csv", sep=";", header=True)
